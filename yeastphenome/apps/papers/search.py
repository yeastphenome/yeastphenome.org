...
def get_search_tags():
    ...
    """
    queryset = Paper.objects.all_valid()
...
def run_search_tag_query(queries):
    ...
    for query in queries:
        ...
        queryset = queryset.filter(
            ...
            | Q(datasets__name__iregex=query)
            | Q(datasets__phenotype__name__iregex=query)
            | Q(datasets__collection__name__iregex=query)
            | Q(data_abstract__iregex=query)
            | Q(pmid__iregex=query)
            | Q(notes__iregex=query)
            | Q(datasets__medium__systematic_name__iregex=query)
            | Q(datasets__conditionset__systematic_name__iregex=query)
        )
