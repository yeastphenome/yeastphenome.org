...
def index(request):
    ...
    if "q" in request.GET:
        ...
        if form.is_valid():
            ...
            conditions_ids = list(conditions.values_list("id", flat=True).order_by().distinct())
            ...
            if tab == "conditions":
                ...
                conditions = conditions.annotate(condition_doses=StringAgg("conditions__dose",
                                                                           delimiter="; ",
                                                                           distinct=True,
                ...
                conditions = conditions.annotate(papers1=StringAgg(agg_field1, delimiter="; ", distinct=True))
                conditions = conditions.annotate(papers2=StringAgg(agg_field2, delimiter="; ", distinct=True))

                agg_field1 = "conditions__conditionset__dataset__phenotype__observable__name"
                agg_field2 = "conditions__medium__dataset__phenotype__observable__name"
                conditions = conditions.annotate(phenotypes1=StringAgg(agg_field1, delimiter="; ", distinct=True))
                conditions = conditions.annotate(phenotypes2=StringAgg(agg_field2, delimiter="; ", distinct=True))

                conditions = conditions.annotate(condition_tags=StringAgg("tags__name", delimiter="; ", distinct=True))

                conditions_page_values = list(conditions.values("id",
                                                                "name",
                                                                "chebi_name",
                                                                "pubchem_name",
                                                                "condition_doses",
                ...
                def merge_papers_phenotypes_names(condition_dict):
                    ...
                    phenotypes = list(itertools.chain.from_iterable([phenotypes1, phenotypes2]))
                    phenotypes = [phenotype for phenotype in phenotypes if not phenotype == ""]
                    phenotypes = list(set(phenotypes))
                    condition_dict["phenotypes"] = truncated_list_as_str(phenotypes, sort=True)

                    names = [condition_dict["chebi_name"], condition_dict["pubchem_name"]]
                    names = [name for name in names if not name == "" and name is not None]
                    condition_dict["other_names"] = "; ".join(names)

                    return condition_dict

                conditions_page_values = [merge_papers_phenotypes_names(paper_dict) for paper_dict in conditions_page_values]
            ...
            phenotypes_ids = list(phenotypes.values_list("id", flat=True).order_by().distinct())
            ...
            if tab == "phenotypes":
                ...
                phenotypes_page = phenotypes_page.annotate(reporters=StringAgg("phenotype__reporter",
                                                                               delimiter="; ",
                                                                               distinct=True,
                                                                               ordering="phenotype__reporter"))
                phenotypes_page = phenotypes_page.annotate(agg_tags=StringAgg("tags__name",
                                                                              delimiter="; ",
                                                                              distinct=True,
                                                                              ordering="tags__name"))
                agg_field = "phenotype__dataset__paper__systematic_name"
                phenotypes_page = phenotypes_page.annotate(agg_papers=StringAgg(agg_field,
                                                                                delimiter="; ",
                                                                                distinct=True,
                                                                                ordering=agg_field))
                agg_field = "phenotype__dataset__conditionset__conditions__type__name"
                phenotypes_page = phenotypes_page.annotate(agg_conditions=StringAgg(agg_field,
                                                                                    delimiter="; ",
                                                                                    distinct=True,
                                                                                    ordering=agg_field))
                phenotypes_page_values = list(phenotypes_page.values("id",
                                                                     "name",
                                                                     "reporters",
                                                                     "agg_papers",
                                                                     "agg_conditions",
            ...
            if tab != "genes":
                ...
                genes = genes.annotate(gene_aliases=StringAgg("aliases__name", delimiter="; ", distinct=True))
                genes_page_obj, page_range = get_paginator(genes, page_number)
                genes_page_obj_values = list(genes_page_obj.object_list.values("id",
                                                                               "systematic_name",
                                                                               "common_name",
                                                                               "gene_aliases",
                                                                               "description"))
            ...
            filter_list = filter_list_qi | Q(medium__conditions__type__in=conditions_ids)
            ...
            datasets_ids = list(datasets.values_list("id", flat=True).order_by().distinct())
            ...
            if tab == "datasets":
                ...
                datasets_page_obj_values = list(datasets_page_obj.object_list.values("id",
            ...
            if tab == "papers":
                ...
                papers_page_values = list(papers_page_obj.object_list.values("id",
                                                                             "systematic_name",
                                                                             "observables_summary",
