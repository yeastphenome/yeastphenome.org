...
class Observable2(MPTTModel):
    ...
    def has_annotated_descendants(self):
        num_descendants = (
            ...
            .exclude(phenotype__dataset__isnull=True)
            .count()
        )
    ...
    def has_annotated_relevant_descendants(self):
        ...
        num_descendants = (
            self.get_descendants(include_self=True).filter(f).filter(g).count()
        )
        return num_descendants > 0
    ...
    def phenotypes(self):
        return (
            apps.get_model("phenotypes", "Phenotype")
            .objects.filter(observable2=self)
            .distinct()
        )
    ...
    def papers(self):
        return (
            apps.get_model("papers", "Paper")
            .objects.filter(dataset__phenotype__observable2=self)
            .exclude(latest_data_status__status__name="not relevant")
            .distinct()
        )
    ...
    def conditiontypes(self):
        ...
        return (
            apps.get_model("conditions", "ConditionType")
            .objects.filter(
                condition__conditionset__dataset__phenotype__observable2=self,
                condition__conditionset__dataset__paper__latest_data_status__status__lt=10,
            )
            .distinct()
        )
    ...
    def datasets(self):
        return (
            ...
            .objects.filter(phenotype__observable2=self)
...
class Phenotype(models.Model):
    ...
    def observable2_name(self):
        return '<span style="white-space: nowrap;">%s %s</span>' % (
            self.ancestry(),
            self.observable2.name,
        )
